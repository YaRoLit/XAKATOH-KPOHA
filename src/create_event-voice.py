...
def finalize_event(tgbot, message, temp, custom_data=''):
    ...
    if(custom_data != ''):
        ...
        event['place'] = custom_data[message.chat.id]['place']
        event['time'] = custom_data[message.chat.id]['time']
        event['tags'] = ['#авто-тег']
        event['title'] = 'Авто-Название'
        event['description'] = 'Авто-описание'
        event['duration'] = custom_data[message.chat.id]['long']
